...
import questionary # for asking the user questions and receiving answers

import Constants # for input paths, output paths, measures and possible user responses
...
def ask_user_for_log_measures(preselection = None):
    ...
    measure_types = dict()
    measure_types['simple_stats'] = []
    measure_types['trace_length'] = []
    measure_types['trace_variant'] = []
    measure_types['activities'] = []
    measure_types['start_activities'] = []
    measure_types['end_activities'] = []
    measure_types['eventropies'] = []
    measure_types['epa_based'] = []
    measure_types['trace_diversity'] = []
    measure_types['distinct_activities'] = []
    measure_types['repetitions'] = []
    measure_types['comparison_based'] = []
    measure_types['dfg_based'] = []
    question = "Please select which measures you would like to analyze."
    while True:
        options = []
        for measure_type in measure_types.keys():
            options += [str(measure_type) + " (" + str(len(measure_types[measure_type])) + " of " + str(len(Constants.all_measures[measure_type])) + " selected)"]
        options += ["I want to analyze all available log measures."]
        options += ["I have selected the measures that I want to analyze."]
        response = questionary.select(question, choices=options).ask()
        if response == "I want to analyze all available log measures.":
            return Constants.simple_stats + Constants.trace_length + Constants.trace_variant + Constants.activities + Constants.start_activities + Constants.end_activities + Constants.eventropies + Constants.epa_based + Constants.trace_diversity + Constants.distinct_activities + Constants.repetitions + Constants.comparison_based + Constants.dfg_based
        elif response == "I have selected the measures that I want to analyze.":
            return measure_types['simple_stats'] + measure_types['trace_length'] + measure_types['trace_variant'] + measure_types['activities'] + measure_types['start_activities'] + measure_types['end_activities'] + measure_types['eventropies'] + measure_types['epa_based'] + measure_types['trace_diversity'] + measure_types['distinct_activities'] + measure_types['repetitions'] + measure_types['comparison_based'] + measure_types['dfg_based']
        else:
            measure_type = response.split(" ")[0]
            measure_types[measure_type] = ask_user_for_specific_log_measures(measure_type, measure_types[measure_type])

def ask_user_for_specific_log_measures(measure_type: str, already_selected: list):
    question = "Which measures of type " + measure_type + " would you like to analyze?"
    options = []
    for measure in Constants.all_measures[measure_type]:
        if measure in already_selected:
            options += [questionary.Choice(measure, checked=True)]
        else:
            options += [questionary.Choice(measure, checked=False)]
    selection = questionary.checkbox(question, options).ask()
    return selection
